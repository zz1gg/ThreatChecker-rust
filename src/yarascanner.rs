...
use crate::filehandler::{bytes_to_string, format_hex_dump};

pub fn scan_with_yara(targetfilebytes: &[u8], yarafilebytes: &[u8]){


    let src = SourceCode::from(&yarafilebytes[..])
        .with_origin("test.yara");

    // Create a compiler.
    let mut compiler = yara_x::Compiler::new();
    compiler.add_source(src).expect("[-] Failed to add source");
    // Add some YARA source code to compile.
    //You can replace it according to your needs or specify it in a command line parameter
...
"#).unwrap();
    ...
    let yara_scan_results = scanner.scan(&*targetfilebytes).unwrap();

    println!("{}",format!("[+] Match Yara Rule: {:?}", yara_scan_results.matching_rules().len()).red());


    // Iterated matching rules
    println!("[+] ScanResults with Yara:");
    for (rule_index, rule_m) in yara_scan_results.matching_rules().enumerate() {
        println!("────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────────");
        println!("[+] ScanResults No.{}", rule_index);
        println!("[+] Matching Yara Rules：  {}", rule_m.identifier());
        println!("[+] Rules NameSpace：  {}", rule_m.namespace());

        for pattern in rule_m.patterns() {
            for jmatch in pattern.matches() {
                let offset = jmatch.data().as_ptr() as usize - targetfilebytes.as_ptr() as usize;
                println!("{}",format!("[!] Identified matching rules bytes at offset:0x{:x} , Length:{} ", offset, jmatch.data().len()).red());

                println!("{}", format_hex_dump(jmatch.data()));

                match bytes_to_string(jmatch.data()) {
                    Ok(s) => {
                        println!("[+] The data string that matches the rule: {}", s);
                    },
                    Err(e) => {
                        println!("[-] The data string that matches the rule with Error: {}", e);
                    }
                }
